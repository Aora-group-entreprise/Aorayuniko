...
function ActionButton({ active, onClick, icon, count }: { active?: boolean; onClick?: () => void; icon: React.ReactNode; count: string }) { return <motion.button className={`action-button ${active ? 'active' : ''}`} whileTap={{ scale: 0.85 }} onClick={onClick}>{icon}<span>{count}</span></motion.button> }
