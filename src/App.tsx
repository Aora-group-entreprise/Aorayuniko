...
function ActionButton({ active, onClick, icon, count }: { active?: boolean; onClick?: () => void; icon: React.ReactNode; count: string }) { return <motion.button className="action-button" style={{ width: 42, transition: 'none', willChange: 'auto' }} whileTap={{ scale: 0.85 }} onClick={onClick}><motion.div className="action-button-circle" style={{ width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.42)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }} animate={active ? { boxShadow: '0 0 14px rgba(255,61,154,0.4)' } : { boxShadow: 'none' }}>{icon}</motion.div><span style={{ color: '#fff', fontSize: 11, fontWeight: 500, lineHeight: 1 }}>{count}</span></motion.button> }
