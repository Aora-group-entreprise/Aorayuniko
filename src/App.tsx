...
import { Bell, Bookmark, ChevronDown, Globe2, Heart, Home, MessageCircle, Plus, Search, Send, User } from 'lucide-react'
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <button className="brand" aria-label="Yuniko home" onClick={() => selectTab('home')}>Yuniko</button>
        ...
        <div className="top-actions">
          ...
          <button aria-label="Profile" onClick={() => selectTab('profile')}><User /></button>
        </div>
      </header>
      ...
      <main className="feed">
        ...
        <article className="post-card">
          <div className="post-media">
            <img src="https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1400&q=90" alt="Ocean sunset" />
            <div className="media-overlay" />

            <div className="side-actions">
              <button className={liked ? 'active' : ''} onClick={() => setLiked(!liked)} aria-label="Like">
                <Heart fill={liked ? 'currentColor' : 'none'} /><span>{liked ? '25.7K' : '25.6K'}</span>
              </button>
              ...
              <button className={saved ? 'active' : ''} onClick={() => setSaved(!saved)} aria-label="Save">
                <Bookmark fill={saved ? 'currentColor' : 'none'} /><span>2.1K</span>
              </button>
            </div>

            <div className="creator-overlay">
              <div className="creator-avatar-wrap">
                <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=160&q=85" alt="Aina" />
              </div>
      ...
      <nav className="bottom-nav" aria-label="Main navigation">
        <button className={active === 'home' ? 'selected' : ''} onClick={() => selectTab('home')}><Home /><span>Home</span></button>
        <button className={active === 'notifications' ? 'selected' : ''} onClick={() => selectTab('notifications')}>
          <span className="nav-icon-wrap"><Bell />{notifications > 0 && <i>{notifications}</i>}</span><span>Notifications</span>
        </button>
        <button className="create" aria-label="Create post"><Plus /></button>
        <button className={active === 'messages' ? 'selected' : ''} onClick={() => selectTab('messages')}><MessageCircle /><span>Messages</span></button>
        <button className={active === 'profile' ? 'selected' : ''} onClick={() => selectTab('profile')}><User /><span>Profile</span></button>
      </nav>
    </div>
  )
}
